      ...
      <ul className="item-list">
        {cart.map(item => (
          <li key={item._id} className="item">
            ...
            <button onClick={() => removeFromCart(item._id)} className="remove-button">Supprimer</button>
          </li>
        ))}
      </ul>
      <div className="total-price">Total: {totalPrice} €</div>
      <button onClick={addCommande} className="remove-button add-commande">Add Commande</button>
    </div>
  );
};
